...
async function getUsdExchangeRates(currencies, year) {
    ...
    const ratePromises = uniqueCurrencies.map(currency => {
        ...
        return getHistoricalAverageRate(currency, 'USD', year).then(rate => ({
            currency,
            rate
        }));
    });
...
function getMockData(companyCode, year) {
    ...
    const mockCompanies = Array.from({
        length: 20
    }, (_, i) => ({
...
app.get("/get-average-rate", async (req, res) => {
    ...
    if (!base || !target || !year) {
        return res.status(400).json({
            error: "Missing required parameters: base, target, year."
        });
    }
    try {
        ...
        if (rate === null) {
            return res.status(404).json({
                error: `Could not calculate an average rate for ${base}-${target} in ${year}.`
            });
        }
        res.json({
        ...
        res.status(500).json({
            error: "Internal server error while fetching rate."
        });
    }
});
...
app.get("/fetch-peers", async (req, res) => {
    ...
    if (!sector || !year) return res.status(400).json({
        error: "Missing parameters"
    });
    ...
    const allCompaniesResponse = deduplicatedPeers.map((c) => ({ ...c,
        isUserCompany: false
    }));
